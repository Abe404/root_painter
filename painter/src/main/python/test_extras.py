...
def test_extract_length(qtbot):
    ...
    def check_output():
        ...
        # csv should contain length for each file in input
...
def test_extract_count(qtbot):
    from extract_count import ExtractCountWidget
    widget = ExtractCountWidget()
    widget.show()
    results_dir = os.path.join(sync_dir, 'projects', 'biopores_corrective_a', 'results')
    seg_dir = os.path.join(results_dir, 'seg_model_33')
    out_csv = os.path.join(results_dir, 'seg_model_33_count.csv')
    ...
    def check_output():
        ...
        # csv should contain count for each file in input
        csv_lines = open(widget.output_csv, 'r').readlines()[1:]
        csv_files = []
        for l in csv_lines:
            parts = l.strip().split(',')
...
def test_extract_region_props(qtbot):
    from extract_regions import ExtractRegionsWidget
    widget = ExtractRegionsWidget()
    widget.show()
    results_dir = os.path.join(sync_dir, 'projects', 'biopores_corrective_a', 'results')
    seg_dir = os.path.join(results_dir, 'seg_model_33')
    out_csv = os.path.join(results_dir, 'seg_model_33_region_props.csv')
    ...
    def check_output():
        ...
        # csv should contain region_props for each file in input
        csv_lines = open(widget.output_csv, 'r').readlines()[1:]
        csv_files = []
        for l in csv_lines:
            parts = l.strip().split(',')
            csv_files.append(parts[0])

        # should have an entry for each file but may have multiple entries
