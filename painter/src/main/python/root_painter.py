...
class RootPainter(QtWidgets.QMainWindow):
    ...
    def open_project(self, proj_file_path):
        ...
        # Workstation mode: check trainer status via instruction
        if self.server_manager:
            project_name = PurePath(settings['location']).name
            proceed, status_info = check_trainer_status(
                self.sync_dir, self.instruction_dir,
                workstation=True, opening_project=project_name,
                parent=self)
            if not proceed:
                return
